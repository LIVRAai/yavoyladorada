...
async function fetchMercadoPagoJson(url: string, accessToken: string) {
  ...
  if (!response.ok) {
    console.error("Mercado Pago resource fetch failed", {
      url,
...
Deno.serve(async (req) => {
  ...
  if (!dataId || !requestId || !signatureParts) {
    return jsonResponse({ ok: false, error: "invalid_webhook_signature_headers" }, 401);
  }
  ...
  if (!timingSafeEqualHex(expectedSignature, signatureParts.v1)) {
    return jsonResponse({ ok: false, error: "invalid_webhook_signature" }, 401);
  }
  ...
  try {
    if (topic === "subscription_preapproval") {
      const subscription = await fetchMercadoPagoJson(
      ...
      if (subscription.preapproval_plan_id) {
        subscriptionRecord.mp_plan_id = subscription.preapproval_plan_id;
      }

      const { error: subscriptionError } = await admin
        .from("business_subscriptions")
        .upsert(subscriptionRecord, { onConflict: "business_id" });

      if (subscriptionError) {
        console.error("Subscription update failed", subscriptionError);
        return jsonResponse({ ok: false, error: "subscription_update_failed" }, 500);
      }

      let businessStatus: string | null = null;

      if (subscription.status === "pending") businessStatus = "pending_payment";
      if (subscription.status === "paused" || subscription.status === "cancelled") {
        businessStatus = "suspended";
      }

      if (businessStatus && business.status !== businessStatus) {
        const { error: statusError } = await admin
          .from("businesses")
          .update({ status: businessStatus, updated_at: now })
          .eq("id", businessId);

        if (statusError) {
          console.error("Business status update failed", statusError);
          return jsonResponse({ ok: false, error: "business_status_update_failed" }, 500);
        }
      }

      return jsonResponse({ ok: true, processed: topic });
    }

    if (topic === "subscription_authorized_payment") {
      const invoice = await fetchMercadoPagoJson(
        `https://api.mercadopago.com/authorized_payments/${encodeURIComponent(resourceId)}`,
        mercadoPagoToken
      );

      if (!invoice.preapproval_id) {
        return jsonResponse({ ok: true, ignored: "authorized_payment_without_subscription" });
      }

      const subscription = await fetchMercadoPagoJson(
        `https://api.mercadopago.com/preapproval/${encodeURIComponent(invoice.preapproval_id)}`,
        mercadoPagoToken
      );
      ...
      if (paymentStatus === "approved") {
        subscriptionRecord.last_payment_date = invoice.debit_date || invoice.last_modified || now;
      }
      ...
      return jsonResponse({ ok: true, processed: topic, payment_status: paymentStatus });
    }

    if (topic === "payment") {
      return jsonResponse({ ok: true, acknowledged: "payment" });
    }
